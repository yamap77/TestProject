	...
	FB.login(
			function(response) {
				if (response.authResponse) {
					...
					//top.location.href = "http://localhost:8888/post?accessToken=" + accessToken;
					/*$.post("http://localhost:8888/post", function(response) {
						  alert("Data Loaded: " + response);
					});*/
					$.post("http://localhost:8888/post", { accessToken: response.authResponse['accessToken']})
					.done(function(data) {
...
// callback function is a JavaScript function you write that is triggered after
// the user authorizes or declines access to the information requested by your
// app.
...
function signinCallback2(authResult) {
	...
	if (authResult['access_token']) {
		...
		// Successfully authorized
		// Hide the sign-in button now that the user is authorized, for example:
		// document.getElementById('signinButton2').setAttribute('style',
		// 'display: none');
		// top.location.href="http://localhost:8888/index.html";
		// top.location.href="http://yamaptest.appspot.com/index.html";
	} else if (authResult['error']) {
